...
def load_css():
    ...
    /* Also force dropdown menu items to have dark background and white text */
    ...
    .stFileUploader > div {
        background: #101112 !important; /* even darker */
        border: 2px dashed #3498db !important; /* brighter border for contrast */
    ...
    .stFileUploader > div:hover {
        box-shadow: 0 0 16px #3498db, 0 4px 24px rgba(0,0,0,0.7) !important;
    }
    /* File uploader 'Browse files' button/text styling */
    .stFileUploader button,
    .stFileUploader [role="button"],
    .stFileUploader label,
    .stFileUploader label *,
    .stFileUploader .uploadStatus,
    .stFileUploader .upload-text,
    .stFileUploader span {
        color: #ffffff !important;
    ...
    /* Robust sidebar background override */
    section[data-testid="stSidebar"] {
        background: linear-gradient(135deg, #1a1a1a 0%, #2d2d2d 25%, #1a1a1a 50%, #2d2d2d 75%, #1a1a1a 100%) !important;
        color: #ffffff !important;
    }
    
    /* Notification styling - higher contrast & highlighted bold text */
    .stToast, .stAlert, .stInfo, .stWarning, .stError, .stSuccess,
    div[data-testid="stAlert"] {
        background: #0b1220 !important; /* deep navy for contrast */
        color: #e6edf6 !important;
        border-radius: 12px !important;
        border: 1px solid #1f2a44 !important;
        box-shadow: 0 4px 18px rgba(0,0,0,0.55) !important;
    }
    /* Distinct colored accents per type with glow */
    .stInfo { border-left: 8px solid #3b82f6 !important; box-shadow: inset 0 0 0 1px rgba(59,130,246,0.15) !important; }
    .stWarning { border-left: 8px solid #f59e0b !important; box-shadow: inset 0 0 0 1px rgba(245,158,11,0.15) !important; }
    .stError { border-left: 8px solid #ef4444 !important; box-shadow: inset 0 0 0 1px rgba(239,68,68,0.15) !important; }
    .stSuccess { border-left: 8px solid #22c55e !important; box-shadow: inset 0 0 0 1px rgba(34,197,94,0.15) !important; }
    
    /* Bold text gets a subtle highlight for readability */
    .stToast strong, .stAlert strong, .stInfo strong, .stWarning strong, .stError strong, .stSuccess strong,
    ...
    .stToast svg, .stAlert svg, .stInfo svg, .stWarning svg, .stError svg, .stSuccess svg,
...
# --- Z-RNA Prediction Function ---
def calculate_z_rna_propensity(sequence):
    """
    Calculate Z-RNA propensity based on alternating purine-pyrimidine patterns
    and GC-rich regions that are prone to Z-RNA formation.
    """
    if not sequence or len(sequence) < 6:
        return 0.0
    ...
    # Check for alternating purine-pyrimidine patterns (Z-RNA prone)
    for i in range(len(sequence) - 5):
        window = sequence[i:i+6]
        if len(window) == 6:
            # Check for alternating pattern
            ...
            if alternating:
                ...
                # Z-RNA is more likely in GC-rich alternating regions
                if gc_content >= 0.5:  # 50% or more GC
                    z_score += gc_content * 2.0  # Higher weight for GC-rich
                else:
                    z_score += gc_content * 1.0  # Lower weight for AU-rich
...
# --- Enhanced Nucleotide Analysis with Z-DNA and Z-RNA ---
def analyze_sequence(name, seq):
    ...
    # Calculate Z-RNA propensity
    z_rna_propensity = calculate_z_rna_propensity(seq)
...
def export_to_fasta(sequences, result_df):
    ...
    for idx, row in result_df.iterrows():
        ...
        description = f"GC Content: {row['GC %']:.2f}%, GT%: {row.get('GT %', 0):.2f}%, CA%: {row.get('CA %', 0):.2f}%, Length: {row['Length']}, "
...
def export_to_genbank(sequences, result_df):
    ...
    for idx, row in result_df.iterrows():
        ...
        output.write('                     /note="GT Content: {:.2f}%"\n'.format(row.get('GT %', 0)))
        output.write('                     /note="CA Content: {:.2f}%"\n'.format(row.get('CA %', 0)))
...
def export_fragments_to_excel(fragments):
    ...
    for frag_name, frag_seq, parent, idx, start_pos, analysis in fragments:
        row = {
            ...
            "GT %": analysis.get("GT %", 0),
            "CA %": analysis.get("CA %", 0),
...
def export_fragments_to_fasta(fragments):
    ...
    for frag_name, frag_seq, parent, idx, frag_len, analysis in fragments:
        ...
        description += f"GC_Content={analysis.get('GC %', 0):.2f}% GT%={analysis.get('GT %', 0):.2f}% CA%={analysis.get('CA %', 0):.2f}% "
...
def export_fragments_to_genbank(fragments):
    ...
    for frag_name, frag_seq, parent, idx, frag_len, analysis in fragments:
        ...
        output.write(f'                     /note="GT Content: {analysis.get("GT %", 0):.2f}%"\n')
        output.write(f'                     /note="CA Content: {analysis.get("CA %", 0):.2f}%"\n')
...
def export_original_and_fragments_zip(result_df, fragment_df, all_fragments, output_filename, fragment_length):
    """Create a ZIP archive containing originals and fragments in multiple formats."""
    signature = "Made by Shubh Rakesh Nahar / Troy University"
    # ensure Error column is not exported
    if "Error" in result_df.columns:
        result_df = result_df.drop(columns=["Error"], errors="ignore")
    zip_buffer = BytesIO()
    with zipfile.ZipFile(zip_buffer, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        # Originals - Excel with basic formatting
        excel_buffer = BytesIO()
        with pd.ExcelWriter(excel_buffer, engine='openpyxl') as writer:
            result_df.to_excel(writer, index=False, sheet_name='GC Content Analysis')
            workbook = writer.book
            worksheet = writer.sheets['GC Content Analysis']
            worksheet.cell(row=1, column=1).comment = openpyxl.comments.Comment(signature, "Author")
            for column in worksheet.columns:
            ...
            worksheet.cell(row=last_row, column=1, value=signature)
        excel_buffer.seek(0)
        zf.writestr(f"{output_filename}_original.xlsx", excel_buffer.read())
        ...
        csv_buffer.seek(0)
        zf.writestr(f"{output_filename}_original.csv", csv_buffer.read())

        # Originals - JSON
        json_data = {
            "metadata": {"signature": signature, "timestamp": pd.Timestamp.now().isoformat()},
            "data": result_df.to_dict(orient='records')
        }
        zf.writestr(f"{output_filename}_original.json", json.dumps(json_data, indent=2))

        # Originals - FASTA & GenBank
        fasta_sequences = [(row["Gene Name"], row["Sequence"]) for _, row in result_df.iterrows()]
        fasta_text = export_to_fasta(fasta_sequences, result_df)
        gb_text = export_to_genbank(fasta_sequences, result_df)
        zf.writestr(f"{output_filename}_original.fasta", fasta_text)
        zf.writestr(f"{output_filename}_original.gb", gb_text)

        # Fragments - only if present
        if fragment_df is not None and len(fragment_df) > 0 and all_fragments:
            # Fragments - Excel
            frag_excel_buffer = export_fragments_to_excel(all_fragments)
            frag_excel_buffer.seek(0)
            zf.writestr(f"{output_filename}_fragments_{fragment_length}bp.xlsx", frag_excel_buffer.read())

            # Fragments - FASTA & GenBank
            frag_fasta_text = export_fragments_to_fasta(all_fragments)
            frag_gb_text = export_fragments_to_genbank(all_fragments)
            zf.writestr(f"{output_filename}_fragments_{fragment_length}bp.fasta", frag_fasta_text)
            zf.writestr(f"{output_filename}_fragments_{fragment_length}bp.gb", frag_gb_text)
            ...
            for _, row in fragment_df.iterrows():
                combined_data.append({
                    ...
                    "Parent Gene": row.get("Parent Gene"),
                    "Fragment Number": row.get("Fragment Number"),
                    "Start Position": row.get("Start Position"),
                    "End Position": row.get("End Position"),
                    ...
                    "CA %": row.get("CA %", 0),
                    "Z-DNA Propensity %": row.get("Z-DNA Propensity %", 0),
                    "A Count": row.get("A Count"),
                    "T Count": row.get("T Count"),
                    "G Count": row.get("G Count"),
                    "C Count": row.get("C Count")
                })
            combined_df = pd.DataFrame(combined_data)
            combined_buffer = BytesIO()
            combined_df.to_excel(combined_buffer, index=False, engine='openpyxl')
            combined_buffer.seek(0)
            zf.writestr(f"{output_filename}_combined_{fragment_length}bp.xlsx", combined_buffer.read())

        # Add a simple manifest
        readme = (
            f"SRNA export bundle\n"
            f"Originals and fragments (if generated) in multiple formats.\n\n"
...
def display_df_with_index_one(df, *args, **kwargs):
    df = df.copy()
    df.index = df.index + 1
...
def main():
    ...
    with st.sidebar.expander("Random Sequence Generator"):
        ...
        if 'random_sequences' in st.session_state and st.session_state['random_sequences']:
            ...
            # remove any transient Error column from outputs
            rand_df = rand_df.drop(columns=["Error"], errors="ignore")
    ...
    with st.container():
        ...
        if sequences:
            # Analysis mode selector: No Fragmentation or specific fragment length
            st.markdown("### Analysis Mode")
            fragment_choice = st.radio(
                "Fragmentation:",
                options=["No Fragmentation", 50, 100, 200, 400],
                index=0,
                help="Choose a fragment length to include fragmentation in results, or select 'No Fragmentation' to analyze originals only"
            )
            do_fragmentation = isinstance(fragment_choice, int)
            fragment_length = fragment_choice if do_fragmentation else None
            ...
            if st.button("Calculate"):
                ...
                if "Error" in result_df.columns:
                    ...
                    result_df = result_df.dropna(subset=["Length"])  # remove invalid rows
                    result_df = result_df.drop(columns=["Error"], errors="ignore")
                else:
                    result_df = result_df.drop(columns=["Error"], errors="ignore")
                ...
                if len(result_df) == 0:
                    ...
                    with st.expander("Show Original Sequences Data", expanded=True):
                        display_df_with_index_one(result_df)

                    # Fragment Analysis Table (only if fragmentation selected)
                    if do_fragmentation and len(fragment_df) > 0:
                        st.markdown(f"### Fragment Analysis ({fragment_length}-base chunks)")
                        with st.expander("Show Fragment Data", expanded=True):
                            display_df_with_index_one(fragment_df)

                    # Export Results section - always shown if there are results
                    st.markdown("---")
                    st.markdown("""
                        <div class="section-container">
                            <h2 class="section-header">SRNA (Smart Rapid Nucleotide Analysis) Export Results</h2>
                        </div>
                    """, unsafe_allow_html=True)
                    ...
                    with col1:
                        excel_buffer = BytesIO()
                        # ensure Error column is not exported
                        safe_result_df = result_df.drop(columns=["Error"], errors="ignore")
                        safe_result_df.to_excel(excel_buffer, index=False, engine='openpyxl')
                        excel_buffer.seek(0)
                        st.download_button(
                    ...
                    with col2:
                        ...
                        safe_df = result_df.drop(columns=["Error"], errors="ignore")
                        fasta_sequences_to_export = [(row["Gene Name"], row["Sequence"]) for _, row in safe_df.iterrows()]
                        fasta_buffer = export_to_fasta(fasta_sequences_to_export, safe_df)
                        st.download_button(
                    ...
                    with col3:
                        ...
                        safe_df_gb = result_df.drop(columns=["Error"], errors="ignore")
                        gb_sequences_to_export = [(row["Gene Name"], row["Sequence"]) for _, row in safe_df_gb.iterrows()]
                        gb_buffer = export_to_genbank(gb_sequences_to_export, safe_df_gb)
                        st.download_button(
                    ...
                    # Fragment Export Section (only when fragmentation is enabled)
                    if do_fragmentation and len(fragment_df) > 0:
                        ...
                        with frag_col4:
                            ...
                            for _, row in result_df.iterrows():
                                combined_data.append({
                                    ...
                                    "CA %": row.get("CA %", 0),
                                    "Z-DNA Propensity %": row.get("Z-DNA Propensity %", 0),
                                    # "Z-RNA Propensity %": row.get("Z-RNA Propensity %", 0),
                                    "A Count": row["A Count"],
                                    "T Count": row["T Count"],
                                    "G Count": row["G Count"],
                                    "C Count": row["C Count"]
                            ...
                            for _, row in fragment_df.iterrows():
                                combined_data.append({
                                    ...
                                    # "Z-RNA Propensity %": row["Z-RNA Propensity %"],
                                    "A Count": row["A Count"],
                                    "T Count": row["T Count"],
                                    "G Count": row["G Count"],
                                    "C Count": row["C Count"]
                    ...
                    # One-click ZIP export bundling originals and fragments (if any)
                    st.markdown("### Download All (ZIP)")
                    zip_buffer = export_original_and_fragments_zip(
                        result_df=result_df,
                        fragment_df=fragment_df if (do_fragmentation and len(fragment_df) > 0) else None,
                        all_fragments=all_fragments if (do_fragmentation and len(all_fragments) > 0) else None,
                        output_filename=output_filename,
                        fragment_length=(fragment_length if do_fragmentation else 0),
                    )
                    st.download_button(
                        label="Download All (ZIP)",
                        data=zip_buffer,
                        file_name=f"{output_filename}_bundle.zip",
                        mime="application/zip",
                        help="Includes originals (Excel/CSV/JSON/FASTA/GenBank) and, if generated, fragments (Excel/FASTA/GenBank) plus a combined Excel"
                    )
    ...
    # Add hoverable Z-DNA/Z-RNA explanation above results
...
#         st.markdown("""
#             <div class="section-container">
#                 <h2 class="section-header">Smart Rapid Nucleotide Analysis</h2>
#             </div>
#         """, unsafe_allow_html=True)
...
#                                     "AT %": analysis.get("AT %", 0),
#                                     "Z-DNA Propensity %": analysis.get("Z-DNA Propensity %", 0),
#                                     # "Z-RNA Propensity %": analysis.get("Z-RNA Propensity %", 0)
#                                 })
...
#                                 # st.metric("Average Z-RNA Propensity", f"{fragment_df['Z-RNA Propensity %'].mean():.2f}%")
                                
#                             with col3:
#                                 st.metric("High Z-DNA Fragments (>50%)", len(fragment_df[fragment_df['Z-DNA Propensity %'] > 50]))
#                                 # st.metric("High Z-RNA Fragments (>50%)", len(fragment_df[fragment_df['Z-RNA Propensity %'] > 50]))
#                                 st.markdown("""
#     <style>
#     .z-hover-explain {
#       position: relative;
...
#       cursor: pointer;
#     }
#     .z-hover-explain .z-hover-content {
#       display: none;
#       position: absolute;
#       left: 0;
#       top: 2em;
...
#       font-size: 0.95em;
#     }
#     .z-hover-explain:hover .z-hover-content {
#       display: block;
#     }
#     </style>
#     <div class="z-hover-explain">
#       <b>Calculations of Z-DNA/ Z-RNA?</b>
#       <div class="z-hover-content">
#         <b>Z-DNA Calculation:</b><br>
#         - Detects alternating purine-pyrimidine patterns (e.g., CGCGCG, GCGCGC) in DNA.<br>
#         - Calculates GC content in each 6-base window.<br>
#         - Higher Z-DNA propensity is assigned to GC-rich, alternating regions.<br>
#         <br>
#         <b>Z-RNA Calculation:</b><br>
#         - Uses the same alternating purine-pyrimidine logic, but for RNA (A, U, G, C).<br>
#         - Looks for GC-rich, alternating regions in RNA sequences.<br>
#         - Higher Z-RNA propensity is assigned to GC-rich, alternating regions.<br>
#         <br>
#         Both scores are normalized and reported as a percentage for each sequence or fragment.
#       </div>
#     </div>
#     """, unsafe_allow_html=True)
...
#         st.markdown("""
#             <div class="section-container">
#                 <h2 class="section-header">SRNA (Smart Rapid Nucleotide Analysis)</h2>
#             </div>
#         """, unsafe_allow_html=True)
